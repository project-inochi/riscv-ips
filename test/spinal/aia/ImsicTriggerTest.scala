...
case class TilelinkImsicTest(hartIds: Seq[Int], sourceIds: Seq[Int], guestIds: Seq[Int], p: tilelink.M2sParameters) extends Component {
  ...
  val imsic = TilelinkImsicTrigger(blocks.flatMap(_.map(_.asImsicFileInfo())), ImsicMapping(), busP)
...
class ImsicTriggerTest extends SpinalSimFunSuite {
  ...
  val infos = (for {hartId <- hartIds; guestId <- guestIds} yield ImsicFileInfo(
      hartId = hartId,
      guestId = guestId,
      sourceIds = sourceIds,
      groupId = 0,
