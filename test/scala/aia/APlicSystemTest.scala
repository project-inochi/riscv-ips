...
//   val rootModes= ArrayBuffer[SpinalEnumElement[aia.APlicSourceMode.type]]()
...
//       rootModes += mode.sorceMode
//       ROOT.mapUpInterrupt(sourceId, node, mode.interruptMode)
//       node
//     })
...
//     val MModes= ArrayBuffer[SpinalEnumElement[aia.APlicSourceMode.type]]()
//     val SModes= ArrayBuffer[SpinalEnumElement[aia.APlicSourceMode.type]]()

//     val sourcesS2Bundles = slave2sourceIds.zip(slaveS1Sources(0).flags).map {
//       case (id, childSource) => {
//         val mode = APlicSimSourceMode.random()
//         SModes += mode.sorceMode
//         S.mapUpInterrupt(id, childSource, mode.interruptMode)
//         }
//     }
...
//       case (id, childSource) => {
//         val mode = APlicSimSourceMode.random()
//         MModes += mode.sorceMode
//         M.mapUpInterrupt(id, childSource)
//         }
//     }
...
//       for ((mode, i) <- dut.rootModes.zipWithIndex) {
//         val isDelegaton = shuffledMCandidates.contains(i)
//         val mode_ = if (isDelegaton) APlicSourceMode.INACTIVE else mode
//         val config = createGateway(mode_, i + 1, agent, masterAddr)
//         config.hartId = 0
//         config.iprio = 1
//         config.setMode(agent, masterAddr, i*4, (if (isDelegaton) 1 else 0))
...
//         var mode = if (MDelegation && !S1Delegation) APlicSimSourceMode.random() else APlicSourceMode.INACTIVE
